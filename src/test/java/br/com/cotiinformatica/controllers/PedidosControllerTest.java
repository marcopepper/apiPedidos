...
public class PedidosControllerTest {
	...
	void testPutPedido_Sucesso() throws Exception {
		...
		mockMvc.perform(put("/api/v1/pedidos/{id}", id).contentType(MediaType.APPLICATION_JSON)
	...
	void testDeletePedido_Sucesso() throws Exception {
		...
		// Act / Asserts
		mockMvc.perform(delete("/api/v1/pedidos/{id}", id)).andExpect(status().isOk())
				.andExpect(jsonPath("$.id").value(response.getId().toString()))
				.andExpect(jsonPath("$.cliente").value(response.getCliente()))
				.andExpect(jsonPath("$.valor").value(response.getValor()))
				.andExpect(jsonPath("$.dataHora").value(response.getDataHora()));
	...
	void testGetAllPedidos_Sucesso() throws Exception {
		...
		// Act / Asserts
		mockMvc.perform(get("/api/v1/pedidos").param("page", "0").param("size", "25").param("sortBy", "id"))
				.andExpect(status().isOk()).andExpect(jsonPath("$.content.length()").value(pedidos.size()));
	}
	...
	void testGetByIdPedido_Sucesso() throws Exception {
		...
		mockMvc.perform(get("/api/v1/pedidos/{id}", id)).andExpect(status().isOk())
				.andExpect(jsonPath("$.id").value(response.getId().toString()))
				.andExpect(jsonPath("$.cliente").value(response.getCliente()))
				.andExpect(jsonPath("$.valor").value(response.getValor()))
				.andExpect(jsonPath("$.dataHora").value(response.getDataHora()));
